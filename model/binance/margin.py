...
class BinanceMargin:
    ...
    def trading(self, exchange, symbol: str, side: int, ProcessingMoney: float):
        ...
        if usdt_balance > 10 and ticker_to_usdt > 10:
            ...
            if side == 1:
                ...
                takep2 = (price / 100 * self.tp2) + price
                takep3 = (price / 100 * self.tp3) + price

                self.trades.takeProfitLong1(exchange, symbol, amount, takep1)
                self.trades.takeProfitLong2(exchange, symbol, amount, takep2)
                self.trades.trailing_market(
                    exchange, symbol, amount, takep3, 'sell')
                ...
                takep2 = price - (price / 100 * self.tp2)
                takep3 = price - (price / 100 * self.tp3)

                self.trades.takeProfitLong1(exchange, symbol, amount, takep1)
                self.trades.takeProfitLong2(exchange, symbol, amount, takep2)
                self.trades.trailing_market(
                    exchange, symbol, amount, takep3, 'buy')
