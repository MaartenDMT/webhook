        ...
        while inPosition == False or ((longPosition == False and side == 1) or (shortPosition == False and side == -1)):
            ...
            if longPosition == False and side == 1:
                ...
                if shortPosition:
                    ...
                    self.logger.info(
                        f"usdtm - Amount Short Exit Position: {amount}")
                ...
                if longPosition:
                    ...
                    self.logger.info(
                        "usdtm -SETTING STOPLOSS FOR LONG POSITION")
                    ...
                    self.trades.stoplossLong(
                        exchange, symbol, stop, get_amount)
            ...
            # BEAR EVENT
                ...
                self.get_amount_s = float(
                    free_balance["USDT"]) / 100 * ProcessingMoney * leverage / float(df["close"][len(df.index) - 1])
                self.logger.info(
                    f"usdtm - ENTERING SHORT POSITION...: {self.get_amount_s}")
                s = self.trades.shortEnter(
                    exchange, symbol, self.get_amount_s, self.trade_info)
                takeprofit1 = False
                takeprofit2 = False
                takeprofit3 = False
                if s == False:
            ...
            if shortPosition:
                ...
                stop = float(position_info["entryPrice"][len(position_info.index) - 1]) / 100 * \
                    stopLoss + \
                    float(position_info["entryPrice"]
                ...
                get_amount = float(position_info["positionAmt"][len(
                    position_info.index) - 1]) / (3 * -1)
                # TAKE PROFIT FOR SHORT POSITION
                # TAKE PROFIT 1
                if takeprofit1 == False:
                    self.logger.info("usdtm - TAKE PROFIT 1")
                    takeps1 = float(position_info["entryPrice"][len(position_info.index) - 1]) - float(
                        position_info["entryPrice"][len(position_info.index) - 1])/100 * tp1
                    takeprofit1 = self.trades.takeProfitShort1(
                        exchange, symbol, get_amount, takeps1)
                    message = f"usdtm - LONG EXIT (TAKE PROFIT 1): {get_amount}\n" + \
                        "Total Money: " + str(balance['total']["USDT"])
                    content = f"Subject: {symbol}\n{message}"
                    self.logger.info(content)
                ...
                if takeprofit2 == False:
                    ...
                    takeprofit2 = self.trades.takeProfitShort2(
                        exchange, symbol, get_amount, takeps2)
                    message = f"usdtm - LONG EXIT (TAKE PROFIT 2): {get_amount}\n" + \
                        "Total Money: " + str(balance['total']["USDT"])
                    content = f"Subject: {symbol}\n{message}"
                    self.logger.info(content)
            ...
            if shortPosition:
                stop_price = round(float(position_info["entryPrice"][len(
                    position_info.index) - 1]) / 100 * stopLoss + float(position_info["entryPrice"][len(position_info.index) - 1]), 2)
                tk1 = round(float(position_info["entryPrice"][len(position_info.index) - 1])-(
                    float(position_info["entryPrice"][len(position_info.index) - 1])/100) * tp1, 2)
                tk2 = round(float(position_info["entryPrice"][len(position_info.index) - 1])-(
                    float(position_info["entryPrice"][len(position_info.index) - 1])/100) * tp2, 2)
                tk3 = round(float(position_info["entryPrice"][len(position_info.index) - 1])-(
                    float(position_info["entryPrice"][len(position_info.index) - 1])/100) * tp3, 2)
                tkp1 = round(float(position_info["entryPrice"][len(
                    position_info.index) - 1]) / 100 * tp1, 2)
                tkp2 = round(float(position_info["entryPrice"][len(
                    position_info.index) - 1]) / 100 * tp2, 2)
                tkp3 = round(float(position_info["entryPrice"][len(
                    position_info.index) - 1]) / 100 * tp3, 2)
                ...
                stop_price = round(float(position_info["entryPrice"][len(position_info.index) - 1])-(
                    float(position_info["entryPrice"][len(position_info.index) - 1])/100) * stopLoss, 2)
                tk1 = round(((float(position_info["entryPrice"][len(position_info.index) - 1]) / 100) * tp1) + float(
                    position_info["entryPrice"][len(position_info.index) - 1]), 2)
                tk2 = round(((float(position_info["entryPrice"][len(position_info.index) - 1]) / 100) * tp2) + float(
                    position_info["entryPrice"][len(position_info.index) - 1]), 2)
                tk3 = round(((float(position_info["entryPrice"][len(position_info.index) - 1]) / 100) * tp3) + float(
                    position_info["entryPrice"][len(position_info.index) - 1]), 2)
                tkp1 = round(
                    (float(position_info["entryPrice"][len(position_info.index) - 1]) / 100) * tp1, 2)
                tkp2 = round(
