...
class TradeCrypto:
  ...
  def trade(self):
    self.execute_usdtm_trade(self.ex[0], self.symbol, self.side, self.t, self.leverage, self.tp1, self.tp2, self.tp3, self.stopLoss, self.ProcessingMoney)
    self.execute_coinm_trade(self.ex[1], self.symbol, self.side, self.t, self.leverage, self.tp1, self.tp2, self.tp3, self.stopLoss, self.ProcessingMoney)
    ...
    while inPosition == False:
      ...
      if shortPosition:
        ...
        if takeprofit3 == False:
          ...
          self.takeProfitShort2(exchange,symbol, get_amount, takeps3)
          takeprofit3 = True
          ticker = symbol
          message = f"LONG EXIT (TAKE PROFIT 3): {get_amount}\n" + \
              "Total Money: " + str(balance['total']["USDT"])
    ...
    while inPosition == False:
      ...
      if longPosition:
        ...
        if takeprofit1 == False:
          ...
          self.logger.info("TAKE PROFIT 1")
          get_amount = float(
              float(position_info["positionAmt"][len(position_info.index) - 1]) / 2)
      ...
      if shortPosition:
        ...
        if takeprofit1 == False:
          ...
          get_amount = (
              float(position_info["positionAmt"][len(position_info.index) - 1]) / 2) * -1
